...
export default function AnimatePage() {
    ...
    // --- Render ---
    const previewHtml = activeTab === 'html' ? typedContent : (activeTab === 'css' || activeTab === 'js' ? code.html : '');
    const previewCss = activeTab === 'css' ? typedContent : (activeTab === 'js' ? code.css : (activeTab === 'html' ? '' : code.css));

    let pHTML = '', pCSS = '', pJS = '';
    switch (activeTab) {
        case 'html': pHTML = typedContent; break;
        case 'css': pHTML = code.html; pCSS = typedContent; break;
        case 'js': pHTML = code.html; pCSS = code.css; pJS = typedContent; break;
    }

    // Idle State: Show full code if not playing and no typed content
    const showPlaceholder = !isPlaying && typedContent.length === 0;
    if (showPlaceholder) {
        pHTML = code.html; pCSS = code.css; pJS = code.js;
    }
