...
export const TypeTabEditor: React.FC<TypeTabEditorProps> = ({
    ...
    return (
        <div className={`flex flex-col w-full h-full overflow-hidden rounded-xl border transition-colors duration-300 ${isLight
            ? 'bg-white border-gray-200'
            : 'bg-[#1e1e1e] border-white/10'
            } ${className || ''}`}>

            {/* Tab Bar */}
            <div className={`flex items-center px-2 h-10 border-b shrink-0 ${isLight ? 'bg-gray-50/50 border-gray-200' : 'bg-[#252525] border-white/5'
                }`}>
                <div className="flex items-center gap-1">
                  ...
                  ${isActive
                                        ? (isLight ? 'bg-white text-gray-900 shadow-sm ring-1 ring-black/5' : 'bg-white/10 text-white shadow-sm ring-1 ring-white/10')
