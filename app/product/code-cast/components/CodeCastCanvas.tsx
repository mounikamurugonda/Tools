...
interface CodeCastCanvasProps {
    layout: any;
    config: AppConfig;
    children: React.ReactNode;
}

export const CodeCastCanvas: React.FC<CodeCastCanvasProps> = ({
    layout,
    config,
    children,
}) => {
    return (
        <div
            id="canvas-stage"
            className={`flex-1 flex flex-col ${getContainerBackgroundClass(config.background)} relative overflow-hidden ${config.isClassicView ? '' : 'shadow border'}`}
            style={{
                ...
                padding: config.isClassicView ? '0px' : `${config.canvasPadding}px`,
            }}
        >
            <BackgroundRenderer background={config.background} />
            {!config.isClassicView && <ProjectTitleDisplay />}

            {/* Content Wrapper */}
            <div className={`flex-1 flex ${layout.flexDirection} ${config.isClassicView ? '' : layout.gap} w-full min-h-0 relative z-10`}>
                {children}
            </div>
