    ...
    const getFilteredFrameOptions = () => {
        if (typeof window !== 'undefined' && window.innerWidth < 768) {
            ...
            return FRAME_OPTIONS.filter(f =>
                f.id === 'tiktok-shorts' || f.id === 'instagram-square' || f.id === 'linkedin-post'
            );
        }
                        ...
                        {isExportMenuOpen && (
                            <>
                                ...
                                <div className="absolute top-full right-0 mt-2 bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 shadow-xl z-20 overflow-hidden">
                                    <button onClick={() => handleDownload('png')} className="w-full px-4 py-2 text-left text-sm hover:bg-gray-50 dark:hover:bg-gray-800">PNG</button>
                                    <button onClick={() => handleDownload('jpg')} className="w-full px-4 py-2 text-left text-sm hover:bg-gray-50 dark:hover:bg-gray-800">JPG</button>
                                    <button onClick={() => handleDownload('svg')} className="w-full px-4 py-2 text-left text-sm hover:bg-gray-50 dark:hover:bg-gray-800">SVG</button>
                                </div>
                            </>
                        )}
                    </div>
                ...
                <div className="relative">
                    ...
                    >
                        ...
                        <span className="hidden md:inline text-[11px]">
                            {FRAME_OPTIONS.find(f => f.id === config.deviceFrame)?.label.split(' ')[0] || 'Device'}
                        </span>
                ...
                {(mode === 'type' || mode === 'animate') && (
                    <div className="flex items-center gap-1">
                        <button
                            ...
                            className={`p-1.5 rounded-md transition-colors ${isMicEnabled ? 'text-red-500 bg-red-50 dark:bg-red-900/20' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'
                                }`}
