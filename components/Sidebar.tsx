...
const Sidebar = () => {
  ...
  return (
    <aside
      data-lenis-prevent
      className="hidden lg:flex flex-col w-64 bottom-0 overflow-y-auto overscroll-y-contain border-r border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 fixed left-0 top-20 z-40"
    >
                    ...
                    {toolsByCategory[category].map(tool => {
                      ...
                      return (
                        <Link
                          key={tool.id}
                          href={`/tools/${tool.id}`}
                          className={`group flex items-center gap-2.5 px-3 py-1.5 text-sm rounded-lg transition-all ${
                            isActive
