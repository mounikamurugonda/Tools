      ...
      <div className="space-y-8 text-center">
        {!event ? (
            <div className="p-12 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-800">
                <p className="text-2xl font-semibold text-gray-500">Press any key on your keyboard</p>
            </div>
        ) : (
            <div className="space-y-8">
                <div className="p-8 bg-blue-50 dark:bg-blue-900/20 rounded-xl inline-block">
                    <span className="block text-sm text-gray-500 dark:text-gray-400 mb-2">JavaScript Key Code</span>
                    <span className="text-8xl font-bold text-blue-600 dark:text-blue-400">{event.keyCode}</span>
                </div>
...
const InfoCard = ({ label, value }: { label: string, value: string }) => (
    <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
        ...
        <span className="text-lg font-mono font-bold text-gray-800 dark:text-gray-200">{value === ' ' ? '(Space)' : value}</span>
    </div>
);

const Badge = ({ label, active }: { label: string, active: boolean }) => (
    <span className={`px-3 py-1 rounded-full text-sm font-medium ${active ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' : 'bg-gray-200 text-gray-500 dark:bg-gray-700 dark:text-gray-400'}`}>
        {label}
    </span>
);
