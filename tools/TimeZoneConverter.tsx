...
const TimeZoneConverter: React.FC<ToolProps> = ({ details, toolId }) => {
    ...
    const timeDifference = useMemo(() => {
        try {
            const now = new Date();
            const fromOffset = new Intl.DateTimeFormat('en-US', { timeZone: fromTimeZone, timeZoneName: 'longOffset' }).formatToParts(now).find(p => p.type === 'timeZoneName')?.value.replace('GMT', '');
            const toOffset = new Intl.DateTimeFormat('en-US', { timeZone: toTimeZone, timeZoneName: 'longOffset' }).formatToParts(now).find(p => p.type === 'timeZoneName')?.value.replace('GMT', '');
    ...
    const result = useMemo(() => {
        ...
        try {
            ...
            return {
                ...
                date: dateInFromZone.toLocaleDateString(undefined, { timeZone: toTimeZone, month: 'long', day: 'numeric', year: 'numeric' }),
                time: dateInFromZone.toLocaleTimeString(undefined, { timeZone: toTimeZone, hour: '2-digit', minute: '2-digit', hour12: true }),
                day: dateInFromZone.toLocaleDateString(undefined, { timeZone: toTimeZone, weekday: 'long' }),
            };
        } catch (e) {
            ...
            <div className="max-w-2xl mx-auto space-y-6">
                <div className="grid md:grid-cols-2 gap-6 items-end">
                    <div>
                        ...
                        <input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded p-2 focus:outline-none focus:ring-2 focus:ring-blue-500" style={{ colorScheme: 'dark' }} />
                    </div>
                     <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Time</label>
                        <input type="time" value={time} onChange={e => setTime(e.target.value)} className="w-full bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded p-2 focus:outline-none focus:ring-2 focus:ring-blue-500" style={{ colorScheme: 'dark' }}/>
                    </div>
                </div>

                <div className="grid md:grid-cols-2 gap-4 items-start">
                    <div className="space-y-4">
                        <TimeZoneSelector label="From" selected={fromTimeZone} onSelect={setFromTimeZone} timezones={timezones} />
                    </div>
                    <div className="space-y-4">
                        <TimeZoneSelector label="To" selected={toTimeZone} onSelect={setToTimeZone} timezones={timezones} />
                    </div>
                </div>
                
                <div className="flex justify-center">
                    <button onClick={handleSwap} className="p-2 bg-gray-200 dark:bg-gray-700 rounded-full hover:bg-gray-300 dark:hover:bg-gray-600" title="Swap timezones">
...
const TimeZoneSelector: React.FC<{ label: string; selected: string; onSelect: (tz: string) => void; timezones: { group: string; zones: { value: string; label: string }[] }[] }> = ({ label, selected, onSelect, timezones }) => (
