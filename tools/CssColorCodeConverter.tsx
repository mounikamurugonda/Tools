...
function rgbaToHex({ r, g, b, a }: { r: number; g: number; b: number; a?: number }, withAlpha = false): string {
...
function formatRgb({ r, g, b, a }: { r: number; g: number; b: number; a: number }) {
  return a < 1 ? `rgba(${r}, ${g}, ${b}, ${a.toFixed(3).replace(/0+$/,'').replace(/\.$/,'')})` : `rgb(${r}, ${g}, ${b})`;
}

function formatHsl({ h, s, l, a }: { h: number; s: number; l: number; a: number }) {
  const sPct = Math.round(s * 100);
  const lPct = Math.round(l * 100);
  return a < 1 ? `hsla(${Math.round(h)}, ${sPct}%, ${lPct}%, ${a.toFixed(3).replace(/0+$/,'').replace(/\.$/,'')})` : `hsl(${Math.round(h)}, ${sPct}%, ${lPct}%)`;
}

function resolveCssColorKeyword(keyword: string): { r: number; g: number; b: number; a: number } | null {
...
const CssColorCodeConverter: React.FC<ToolProps> = ({ details, toolId }) => {
  // base state is RGBA
  const [rgba, setRgba] = useState<{ r: number; g: number; b: number; a: number }>({ r: 59, g: 130, b: 246, a: 1 });
  ...
  const derived = useMemo(() => {
    const hex = rgbaToHex({ r: rgba.r, g: rgba.g, b: rgba.b, a: Math.round(rgba.a * 255) }, rgba.a < 1);
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="brand-card p-4 flex items-center justify-center">
            <div className="w-40 h-40 rounded-xl border border-gray-200 dark:border-gray-700 shadow-inner" style={{ backgroundColor: `rgba(${rgba.r}, ${rgba.g}, ${rgba.b}, ${rgba.a})` }} />
          </div>
          <div className="md:col-span-2 brand-card p-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold mb-2">HEX / HEXA</label>
                <div className="flex gap-2">
                  ...
                  />
                  <button className="brand-button-secondary whitespace-nowrap" onClick={() => copy(hexInput)}>Copy</button>
                </div>
              </div>
              <div>
                <label className="block text-sm font-semibold mb-2">RGB / RGBA</label>
                <div className="flex gap-2">
                  <input
                    value={rgbInput}
                    ...
                    className="brand-input font-mono"
                  />
                  <button className="brand-button-secondary whitespace-nowrap" onClick={() => copy(rgbInput)}>Copy</button>
                </div>
              </div>
              <div>
                <label className="block text-sm font-semibold mb-2">HSL / HSLA</label>
                <div className="flex gap-2">
                  <input
                    value={hslInput}
                    ...
                    className="brand-input font-mono"
                  />
                  <button className="brand-button-secondary whitespace-nowrap" onClick={() => copy(hslInput)}>Copy</button>
                </div>
              </div>
              <div>
                <label className="block text-sm font-semibold mb-2">Alpha</label>
                <div className="flex items-center gap-4">
                  <input type="range" min={0} max={1} step={0.01} value={rgba.a} onChange={(e) => setRgba(prev => ({ ...prev, a: parseFloat(e.target.value) }))} />
                  <span className="w-12 text-right tabular-nums">{rgba.a.toFixed(2)}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="brand-card p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
            <div>
              <label className="block text-sm font-semibold mb-2">CSS Color Keyword</label>
              <input
                value={keywordInput}
              ...
              />
              <p className="brand-text-muted mt-2">Enter a CSS color keyword and click Resolve to convert to HEX/RGB/HSL.</p>
            </div>
            <div className="flex gap-2 md:justify-end">
              <button className="brand-button-primary" onClick={handleKeywordResolve}>Resolve</button>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="brand-card p-4">
            ...
            <div className="flex items-center gap-2">
              <code className="px-2 py-1 rounded bg-gray-100 dark:bg-gray-700 font-mono text-sm">{derived.hex}</code>
              <button className="brand-button-secondary" onClick={() => copy(derived.hex)}>Copy</button>
            </div>
          </div>
          <div className="brand-card p-4">
            <div className="text-sm font-semibold mb-2">RGB</div>
            <div className="flex items-center gap-2">
              <code className="px-2 py-1 rounded bg-gray-100 dark:bg-gray-700 font-mono text-sm">{derived.rgb}</code>
              <button className="brand-button-secondary" onClick={() => copy(derived.rgb)}>Copy</button>
            </div>
          </div>
          <div className="brand-card p-4">
            <div className="text-sm font-semibold mb-2">HSL</div>
            <div className="flex items-center gap-2">
              <code className="px-2 py-1 rounded bg-gray-100 dark:bg-gray-700 font-mono text-sm">{derived.hsl}</code>
              <button className="brand-button-secondary" onClick={() => copy(derived.hsl)}>Copy</button>
            </div>
          </div>
        </div>
      </div>
