...
const convertSheetToCsv = (
  ...
  const escapeCell = (cell: any): string => {
    ...
    if (options.quoteFields === 'always') {
      ...
      needsQuotes =
        finalStr.includes(options.separator) ||
        finalStr.includes('"') ||
        finalStr.includes('\n');
    }
  ...
  return data
    .map((row) => row.map(escapeCell).join(options.separator))
    .join('\n');
};
...
const CsvXlsxConverter: React.FC<ToolProps> = ({ details, toolId }) => {
  ...
  const handleConvertAndDownload = async () => {
    try {
      ...
      if (!sourceCsv.trim()) {
        setError(
          'Input data is empty or not yet processed. Please wait or check your input.',
        );
        return;
      }
      ...
      const fileName =
        (isPasted
          ? 'converted'
          : uploadedFile?.name.split('.')[0] || 'converted') +
    ...
    if (['xlsx', 'xls'].includes(fileExtension || '')) {
      ...
      reader.onload = async (event) => {
        try {
          ...
          setError(
            err instanceof Error ? err.message : 'Failed to read XLSX file.',
          );
        }
      };
      ...
      setError(
        'Unsupported file type. Please upload a .csv, .txt, .xlsx, or .xls file.',
      );
      setUploadedFile(null);
    }
  }, []);
                    ...
                    <div className="grid sm:grid-cols-2 gap-6">
                      <div>
                        ...
                        <Select
                          id="separator"
                          value={{
                            value: csvToXlsxSeparator, label: [
                              { value: ",", label: "Comma (,)" },
                          ...
                          onChange={(option) =>
                            setCsvToXlsxSeparator(option?.value as string)
                          }
                          options={[
                            { value: ",", label: "Comma (,)" },
                            { value: ";", label: "Semicolon (;)" },
                        ...
                        {csvToXlsxSeparator === 'custom' && (
                          <Input
                            ...
                            onChange={(e) =>
                              setCsvToXlsxCustomSeparator(e.target.value)
                            }
                            className="mt-2"
                    ...
                    <div className="grid sm:grid-cols-2 gap-6">
                      <div>
                        ...
                        <Select
                          id="xlsx-separator"
                          value={{
                            value: xlsxToCsvSeparator, label: [
                              { value: ",", label: "Comma (,)" },
                              { value: ";", label: "Semicolon (;)" },
                              { value: "\\t", label: "Tab" },
                              { value: "|", label: "Pipe (|)" },
                          ...
                          onChange={(option) =>
                            setXlsxToCsvSeparator(option?.value as string)
                          }
                          options={[
                            { value: ",", label: "Comma (,)" },
                            { value: ";", label: "Semicolon (;)" },
                        ...
                        {xlsxToCsvSeparator === 'custom' && (
                          <Input
                            ...
                            onChange={(e) =>
                              setXlsxToCsvCustomSeparator(e.target.value)
                            }
                            placeholder="Custom separator"
                            className="mt-2"
                          />
                      ...
                      <div>
                        ...
                        <Select
                          id="quoteFields"
                          value={{ value: quoteFields, label: quoteFields.charAt(0).toUpperCase() + quoteFields.slice(1) }}
                          onChange={(option) =>
                            setQuoteFields(option?.value as QuoteFields)
                          }
                          options={[
