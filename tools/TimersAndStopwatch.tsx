      ...
      <div className="max-w-md mx-auto">
        <div className="flex justify-center bg-gray-200 dark:bg-gray-700 rounded-lg p-1 mb-6">
          <button
            ...
            className={`w-1/2 py-2 rounded-md transition-colors ${mode === 'timer' ? 'bg-blue-600 text-white' : 'hover:bg-gray-300 dark:hover:bg-gray-600'}`}
          >
            Timer
          </button>
          <button
            onClick={() => setMode('stopwatch')}
            className={`w-1/2 py-2 rounded-md transition-colors ${mode === 'stopwatch' ? 'bg-blue-600 text-white' : 'hover:bg-gray-300 dark:hover:bg-gray-600'}`}
          >
            Stopwatch
          </button>
        </div>
    ...
    <div className="flex flex-col items-center space-y-6">
      ...
      <div className="flex space-x-4">
        {!isActive ? (
          ...
          <button
            onClick={handlePause}
            className="px-8 py-3 bg-yellow-600 hover:bg-yellow-700 text-white rounded-lg font-bold text-lg w-32"
          >
            Pause
          </button>
        )}
    ...
    <div className="flex flex-col items-center space-y-6">
      ...
      <div className="flex space-x-4">
        {!isActive ? (
          ...
          <button
            onClick={handleStop}
            className="px-8 py-3 bg-red-600 hover:bg-red-700 text-white rounded-lg font-bold text-lg w-32"
          >
            Stop
          </button>
        )}
        ...
        <button
          onClick={handleLap}
          disabled={!isActive}
          className="px-8 py-3 bg-green-600 hover:bg-green-700 text-white rounded-lg font-bold text-lg w-32 disabled:bg-gray-400 dark:disabled:bg-gray-500 disabled:cursor-not-allowed"
        >
          Lap
        </button>
      </div>
        ...
        {laps
          ...
          .map((lap, i) => {
            ...
            return (
              <div
                key={i}
                className="flex justify-between items-center bg-gray-100 dark:bg-gray-700 p-2 rounded-md font-mono text-sm text-gray-800 dark:text-gray-200"
              >
