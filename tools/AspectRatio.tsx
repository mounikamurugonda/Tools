    ...
    <ToolContainer title="Aspect Ratio Calculator" details={details} toolId={toolId}>
      <div className="space-y-8 max-w-lg mx-auto text-center">
        <div className="flex items-center justify-center gap-4 text-4xl font-bold text-blue-600 dark:text-blue-400">
            <input type="number" value={width} onChange={e => setWidth(Number(e.target.value))} className="w-32 text-center bg-transparent border-b-2 border-gray-300 focus:border-blue-500 outline-none" />
            <span>:</span>
            <input type="number" value={height} onChange={e => setHeight(Number(e.target.value))} className="w-32 text-center bg-transparent border-b-2 border-gray-300 focus:border-blue-500 outline-none" />
        </div>
        
        <div className="p-6 bg-gray-100 dark:bg-gray-800 rounded-xl">
            <p className="text-gray-500 uppercase tracking-wide text-sm font-semibold">Aspect Ratio</p>
