...
const csvToJson = (csv: string, options: CsvToJsonOptions): any[] => {
  ...
  if (lines.length < 2) {
    if (lines.length === 1 && lines[0])
      throw new Error(
        "CSV must have at least one header row and one data row for 'Array of Objects' format.",
      );
    return [];
  }
            ...
            <div>
              ...
              {separator === 'custom' && (
                <input
                  ...
                  className="mt-2 w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded p-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
