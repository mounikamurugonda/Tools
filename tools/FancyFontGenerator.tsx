...
const FONTS: Record<string, (s: string) => string> = {
    'Bold': (s) => s.replace(/[a-zA-Z0-9]/g, (c) => {
    ...
    'Typewriter': (s) => s.replace(/[a-zA-Z0-9]/g, (c) => {
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4">
            {Object.entries(FONTS).map(([name, transform]) => (
                <div key={name} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                    <div>
                        <span className="text-xs text-gray-500 uppercase tracking-wide block mb-1">{name}</span>
