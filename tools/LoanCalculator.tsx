...
const LoanCalculator: React.FC<ToolProps> = ({ details, toolId }) => {
    ...
    const { monthlyPayment, totalInterest, totalPayment, schedule } = useMemo(() => {
        ...
        if (isNaN(P) || P <= 0 || isNaN(annualRate) || annualRate < 0 || isNaN(termInYears) || termInYears <= 0) {
        ...
        if (i === 0) { // Interest-free loan
             const M = P / n;
             return { monthlyPayment: M, totalInterest: 0, totalPayment: P, schedule: [] }; // Schedule generation for 0% is simple, but let's omit for now.
        }
        ...
        return { monthlyPayment: M, totalInterest, totalPayment, schedule: scheduleData };
        ...
        <ToolContainer title="Loan Calculator" details={details} toolId={toolId}>
            <div className="grid lg:grid-cols-3 gap-6">
                <div className="lg:col-span-1 space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Loan Term</label>
                        <div className="flex">
                            <input
                                type="number"
                                value={loanTerm}
                     ...
                     <div className="pt-4 space-y-4">
                        <SummaryCard label="Monthly Payment" value={formatCurrency(monthlyPayment)} />
                        <SummaryCard label="Total Interest Paid" value={formatCurrency(totalInterest)} />
                        <SummaryCard label="Total Cost of Loan" value={formatCurrency(totalPayment)} />
                    </div>
                </div>
                            ...
                            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                {schedule.map(row => (
                                    <tr key={row.month}>
                                        ...
                                        <td className="px-4 py-2 text-right text-green-600 dark:text-green-400">{formatCurrency(row.principal)}</td>
                                        <td className="px-4 py-2 text-right text-red-600 dark:text-red-400">{formatCurrency(row.interest)}</td>
                                        <td className="px-4 py-2 text-right font-semibold">{formatCurrency(row.balance)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
...
const NumberInput: React.FC<{ label: string; value: string; onChange: (v: string) => void; step?: string; }> = ({ label, value, onChange, step = "1" }) => (
    <div>
...
const SummaryCard: React.FC<{ label: string; value: string; }> = ({ label, value }) => (
     <div className="bg-gray-100 dark:bg-gray-700/50 p-4 rounded-lg">
        ...
        <p className="text-2xl font-bold text-blue-500 dark:text-blue-400">{value}</p>
    </div>
);
