      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2 relative">
            ...
            <textarea 
                ...
                placeholder="Result..." 
            />
            {output && <CopyButton textToCopy={output} className="absolute top-8 right-2" />}
        </div>
      </div>
      <div className="mt-4 flex items-center justify-between">
        <button onClick={process} className="px-6 py-2 bg-green-600 hover:bg-green-700 text-white rounded font-medium">Remove Duplicates</button>
        {output && <span className="text-gray-600 dark:text-gray-400">Removed {removedCount} duplicates</span>}
      </div>
    </ToolContainer>
  );
};
