        ...
        {password && (
            <div className="space-y-2">
                ...
                <div className="h-4 bg-gray-200 dark:bg-gray-800 rounded-full overflow-hidden">
                    <div className={`h-full transition-all duration-500 ${color}`} style={{ width: `${width}%` }}></div>
                </div>
                <ul className="text-sm text-gray-500 dark:text-gray-400 mt-4 space-y-1">
                    ...
                    <li className={/[A-Z]/.test(password) ? 'text-green-500' : ''}>• Contains uppercase letter</li>
                    <li className={/[0-9]/.test(password) ? 'text-green-500' : ''}>• Contains number</li>
                    <li className={/[^A-Za-z0-9]/.test(password) ? 'text-green-500' : ''}>• Contains symbol</li>
                </ul>
            </div>
        )}
      </div>
