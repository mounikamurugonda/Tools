...
// Background Presets
const BACKGROUNDS = [
  { name: 'Candy', value: 'linear-gradient(140deg, rgb(255, 207, 115), rgb(255, 122, 47))' },
  { name: 'Ocean', value: 'linear-gradient(225deg, #FF3CAC 0%, #784BA0 50%, #2B86C5 100%)' },
  ...
  const handleExport = async (format: 'png' | 'jpeg' | 'svg' | 'copy') => {
    ...
    try {
      ...
      if (format === 'png') {
        ...
        const blob = await toPng(exportRef.current, { ...options, type: 'image/png' });
        ...
        <div className="w-full lg:w-80 flex-shrink-0 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
                <label className="block text-xs font-medium text-gray-700 dark:text-gray-300">Theme</label>
                <div className="grid grid-cols-2 gap-2">
                  {THEMES.map((t) => (
                    ...
                    >
                      <div className="w-3 h-3 rounded-full border border-black/10" style={{ background: t.bg }}></div>
                      {t.name}
                    </button>
                  ))}
                </div>
            ...
            <div className="space-y-3">
              <label className="block text-xs font-medium text-gray-700 dark:text-gray-300">Background</label>
              <div className="grid grid-cols-5 gap-2">
                {BACKGROUNDS.map((bg, i) => (
                  ...
                  >
                    {background === bg.value && <CheckCircle2 className="w-full h-full text-white p-1 drop-shadow-md" />}
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
                 <div>
                    <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">Padding</label>
                    <input 
                        type="range" 
                        min="16" 
                        max="128" 
                 ...
                 <div>
                    <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">Shadow</label>
                    <input 
                        type="range" 
                        min="0" 
                        max="60" 
             ...
             <div className="space-y-3">
                <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-700 dark:text-gray-300">Window Controls</span>
                    <label className="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" checked={showControls} onChange={() => setShowControls(!showControls)} className="sr-only peer" />
                        <div className="w-9 h-5 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
                    </label>
                </div>
                
                <div>
                    <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Window Title</label>
                    <input 
                        type="text"
                        value={windowTitle} 
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
                ...
                <label className="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" checked={showLineNumbers} onChange={() => setShowLineNumbers(!showLineNumbers)} className="sr-only peer" />
                    <div className="w-9 h-5 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
                </label>
            </div>
          </div>
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="px-4 py-3 flex items-center relative bg-black/10 dark:bg-white/5 border-b border-white/5">
                                ...
                                <div className="mx-auto text-xs font-medium opacity-60 select-none" style={{ color: theme.text }}>
                                    {windowTitle}
                                </div>
                            </div>
                                    ...
                                    >
                                        {code.split('\n').map((line, i) => (
                                            <div key={i} className="table-row">
                                                {showLineNumbers && (
                                                    <span className="table-cell text-right pr-4 select-none opacity-30 w-10 border-r border-white/10 mr-4" style={{ color: theme.text }}>
                                                        {i + 1}
                                                    </span>
                                                )}
                                                <span 
                                                    ...
                                                    dangerouslySetInnerHTML={{ 
                                                        __html: Prism.highlight(line || ' ', Prism.languages[language] || Prism.languages.javascript, language) 
                                                    }} 
                                                />
                                            </div>
                                        ))}
                ...
                {isExporting && (
                    <div className="absolute inset-0 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm z-50 flex items-center justify-center rounded-xl">
                        <div className="bg-white dark:bg-gray-800 px-6 py-4 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 flex items-center gap-3">
                            ...
                            <span className="font-medium text-gray-900 dark:text-white">Generating image...</span>
                        </div>
                    </div>
                )}
            </>
