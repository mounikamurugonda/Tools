      ...
      <div className="space-y-8">
        ...
        <div className="grid md:grid-cols-4 gap-6">
            <div>
                ...
                <input type="range" min="1" max="5" value={layers} onChange={e => setLayers(Number(e.target.value))} className="w-full" />
            </div>
            <div>
                <label className="block text-sm font-medium mb-1">Complexity</label>
                <input type="range" min="2" max="20" value={complexity} onChange={e => setComplexity(Number(e.target.value))} className="w-full" />
            </div>
            <div>
            ...
            <div>
                ...
                <button onClick={() => setSeed(Math.random() * 100)} className="w-full py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Regenerate</button>
            </div>
        </div>
