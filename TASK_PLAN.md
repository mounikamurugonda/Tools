...
- [x] aspect-ratio-calculator — new **Solve mode** (given target ratio + one side, compute the other), 8 ratio presets + 6 size presets, swap, decimal ratio output, copy with toast, replaced off-theme `bg-secondary/50` / `text-muted-foreground` / `text-primary` tokens with sibling gray/blue.
- [x] svg-to-data-uri — 3 encoding modes (**Optimized for CSS** / URL-encoded / Base64) with byte-cost delta %, file upload (.svg up to 5MB), Copy URI + Copy CSS background + Copy `<img>` snippets, useToast, replaces legacy CopyButton, kept live CSS-background preview.
- [x] youtube-thumbnail — **robust ID extractor** handles `watch?v=`, `youtu.be/`, `/shorts/`, `/embed/`, `/live/`, `youtube-nocookie.com`, and bare 11-char IDs (old regex missed shorts/embed/live entirely); **blob-based download** (fetches the JPEG and uses `URL.createObjectURL` — old `<a download>` was ignored by browsers for cross-origin `img.youtube.com` URLs); explicit resolution sizes (1280×720, 640×480, etc.) shown above each card; Copy-URL button next to Download; useToast.
- [x] image-filters — added **Saturate** + **Invert** sliders (8 filters total), **9 preset chips** (Original/Clarendon/Moon/Lark/Sepia/Vintage/Cool/Warm/Invert) with active-state, **Copy CSS filter** button (`filter: …;`), JPEG export alongside PNG (with white BG to flatten alpha for JPEG), useToast on every action, fixed `img.naturalWidth`/`naturalHeight` so canvas matches source resolution exactly, `crossOrigin='anonymous'` to avoid tainted-canvas issues.
- [x] svg-blob-generator — **seeded PRNG (mulberry32)** so the same seed actually reproduces the same blob (previous `seed` state was declared but never wired into `Math.random` — Randomize "worked" only because the memo dep changed); **gradient mode** with second color stop; 8 color presets; Copy SVG + Copy Data URI + Copy CSS + Download `.svg`; useToast; replaced legacy CopyButton.
- [x] svg-wave-generator — **flip orientation** (top vs bottom wave), new **Amplitude** slider, deterministic seed, accessible radiogroup for orientation, Copy SVG + Copy CSS + Download; useToast; replaced legacy CopyButton.
- [x] lorem-image-generator — **deterministic seed mode** (`picsum.photos/seed/{seed}/W/H` — gives the same image for a given seed; previous `?random=` was just cache-bust); 6 aspect-ratio size presets; **HTML + Markdown snippet copy** alongside URL; blob-based download; useToast.
- [x] image-to-base64 — added **Data URL vs Raw base64** toggle (raw without the `data:…;base64,` prefix is useful for some workflows); Copy + Copy `<img>` + Copy CSS; **size delta footer** showing original bytes → base64 bytes (+33%); useToast; replaces legacy CopyButton.
- [x] base64-to-image — **accepts raw base64 too** (auto-sniffs MIME from PNG/JPEG/GIF/WebP/SVG magic bytes and prepends a data URL — previous tool only accepted `data:image/…` and rejected raw base64 outright); proper input validation; `<img onError>` surfaces decode failures as a banner (previously broken-image icon with no explanation); decoded-size footer; useToast on download.
- [x] watermark-adder — **new Text watermark mode** (the SEO copy implied text was supported but only image mode existed) with fill + outline colors and stroke width; **Rotation slider** (works for both modes); **JPEG export option** alongside PNG; canvas uses `naturalWidth`/`naturalHeight` to preserve source resolution; useToast.
- [x] image-resizer — **Output format picker** (Auto/JPEG/PNG/WebP — previously hard-coded JPEG, lost PNG transparency on any PNG input); **accurate output filesize via `canvas.toBlob`** instead of estimating from base64 length; size-delta footer (+/− %); quality slider auto-shows "PNG · lossless" when PNG is selected; useToast.
- [x] image-converter — **swapped BMP for AVIF** (canvas `toDataURL('image/bmp')` silently returns PNG in all browsers — was misleading); **`toBlob` path detects unsupported formats** and surfaces a toast when the browser falls back to a different MIME; **JPEG/AVIF background color picker** (alpha-less formats need a bg — was hard-coded white); accurate output filesize + change-% in details card; useToast.
- [x] image-compressor — switched estimated size (base64 length math) to **accurate `canvas.toBlob` byte count**; "size increased" red warning when recompressing a lossy source grows the file (was silently misleading); useToast on download.
- [x] qr-code-generator — added **size/margin sliders, foreground/background color pickers, error-correction level (L/M/Q/H) selector with explanations**, **SVG export** (vector) alongside PNG plus Copy-SVG, blob-based downloads, useToast; preview wrapper uses the chosen background color.
...
### 2026-06-01 — IMAGE sweep COMPLETE (14 tools)
All 14 IMAGE tools shipped in one session. Real correctness fixes this round: **youtube-thumbnail**'s ID regex missed `/shorts/`, `/embed/`, `/live/`, and `youtube-nocookie.com` URLs entirely (and its download anchor was a no-op because `<a download>` is ignored for cross-origin URLs — fixed with a `fetch → blob → objectURL` path); **base64-to-image** rejected raw base64 outright (now sniffs PNG/JPEG/GIF/WebP/SVG magic bytes and auto-prefixes a data URL) and surfaces decode failures via `<img onError>` instead of showing a bare broken-image icon; **svg-blob-generator**'s `seed` state was declared but never wired into the RNG (path used raw `Math.random` — "Randomize" only "worked" because the memo dep changed, so identical settings still gave different blobs on every re-render) — replaced with a seeded `mulberry32` PRNG; **image-resizer** was hard-coded to JPEG output, silently destroying transparency on every PNG input — now has an Auto/JPEG/PNG/WebP picker; **image-converter** offered BMP that all browsers silently downgrade to PNG via `toDataURL` — swapped for AVIF and the new `toBlob` path detects when the browser returns a different MIME and toasts an explicit "AVIF not supported — got PNG" warning; **image-compressor** estimated output size from base64-string length (wrong; padding-correction was 2 bytes max but the real divergence is hundreds of KB) — switched to `canvas.toBlob` for accurate byte counts and added a "size increased" warning when recompressing a lossy JPEG grows the file. Feature expansions: **aspect-ratio-calculator** got a new Solve-Dimension mode (target ratio + one side → other side, the most-asked use case); **svg-to-data-uri** added Optimized-for-CSS encoding (only escapes the chars that break in CSS `url()`, smallest payload) alongside URL-encoded and Base64, with byte-delta %; **image-filters** added Saturate + Invert sliders plus 9 Instagram-style preset chips (Clarendon/Moon/Lark/Sepia/Vintage/Cool/Warm/Invert) and a Copy-CSS-filter button; **watermark-adder** gained a Text mode (fill + outline + stroke width) and a rotation slider that works for both image and text; **qr-code-generator** went from "text in, png out" to size/margin/colors/error-correction-level with SVG vector export. Cross-cutting wins: every tool now uses `useToast` (replaces 6 legacy `CopyButton` instances and several silent clipboard catches), every export route goes through `canvas.toBlob` (real byte counts, accurate downloads), and the file-upload primitive from Phase 0 picks up another caller (svg-to-data-uri). **tsc clean, `next build` green (180 pages), `check:tool-details` passes (only the known gif-maker VIDEO-sweep warning remains). 80/94 tools done.** Next: VIDEO (7 tools — final big category before MISC).
